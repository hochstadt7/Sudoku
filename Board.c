...
void print_board(Board* board){
    ...
    blocks_per_row = board->dimension / board->col_per_block;
    num_dash=4*board->dimension+board->col_per_block+1;
    for (index_row = 0; index_row < board->dimension; index_row++) {
        if (index_row % board->row_per_block == 0) {
        ...
        for (index_block = 0; index_block < blocks_per_row; index_block++) {
            printf("| ");
            for (index_col = 0; index_col < board->col_per_block; index_col++) {
                if (board->arr[index_row][index_col + board->col_per_block * index_block] != 0) {

                    if (board->fixed[index_row][index_col + board->col_per_block * index_block] != 0)
                        printf(" %2d.", board->arr[index_row][index_col + board->col_per_block * index_block]);
                    else if(board->error[index_row][index_col + board->col_per_block * index_block] != 0){
                    printf(" %2d*", board->arr[index_row][index_col + board->col_per_block * index_block]);
                    }
                    else
                        printf(" %2d ", board->arr[index_row][index_col + board->col_per_block * index_block]);
